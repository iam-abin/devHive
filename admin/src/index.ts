...
const start = async () => {
    ...
    if (!appConfig.MONGO_URL_ADMIN) throw new Error('MONGO_URL_ADMIN must be defined');

    // to connect to mongodb
    await connectDB();

    const candidateProfileUpdatedEvent = new CandidateProfileUpdatedEventConsumer(kafkaClient);
    const recruiterProfileUpdatedEvent = new RecruiterProfileUpdatedEventConsumer(kafkaClient);
