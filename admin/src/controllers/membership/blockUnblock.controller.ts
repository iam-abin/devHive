...
export = (dependencies: IDependency) => {
    const {
        useCases: { getMemberShipPlanByIdUseCase, blockUnblockMemberShipPlanUseCase },
    } = dependencies;

    return async (req: Request, res: Response) => {
        const { id: membershipPlanId } = req.params;

        const job = await getMemberShipPlanByIdUseCase(dependencies).execute(membershipPlanId);
        if (!job) throw new NotFoundError();

        const response = await blockUnblockMemberShipPlanUseCase(dependencies).execute(membershipPlanId);

        const membershipPlan = await getMemberShipPlanByIdUseCase(dependencies).execute(membershipPlanId);

        if (response?.deletedCount === 1) {
            const memberShipPlanDeletedEvent = new MemberShipPlanUpdatedEventPublisher(kafkaClient);
