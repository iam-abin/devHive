...
import { ICandidateProfile, IResume } from '../../types/candidate';
...
export = {
    ...
    createCandidateProfile: async (userData: IUser): Promise<ICandidateDocument> => {
    ...
    uploadResume: async (profileId: string, resume: IResume): Promise<ICandidateDocument | null> => {
        const candidate = await CandidateProfileModel.findByIdAndUpdate(
            profileId,
            { $set: { resume } },
            { new: true },
    ...
    getAllCandidates: async (skip: number, limit: number): Promise<ICandidateDocument[] | []> => {
