...
export = (dependencies: IDependency) => {
    ...
    return async (req: Request, res: Response) => {
        ...
        if (!isExistingUser) throw new BadRequestError('Invalid email or password');

        if (isExistingUser.role !== ROLES.ADMIN) throw new BadRequestError('Invalid Admin');
        ...
        res.status(200).json({
            ...
            data: { user: isExistingUser, accessToken, refreshToken },
        });
    };
};
